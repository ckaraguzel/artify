...
def train(config):
    ...
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='min', factor=0.5, patience=config.PATIENCE // 2, verbose=True)
    
    # Early stopping parameters
    patience = config.PATIENCE  # Number of epochs to wait for improvement
    best_val_loss = float('inf')
    epochs_no_improve = 0
    current_lr = config.LEARNING_RATE  # Track the learning rate
